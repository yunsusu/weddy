...
export default function DashBoard({ data,memberData,setCard,num }: DashBoardProps) {
  ...
  const [filteredItems, setFilteredItems] = useState(
    data.smallCatItems.sort((a, b) => Number(b.statusName) -  Number(a.statusName)) // statusName을 기준으로 정렬
  );
  ...
  const { mutate: changeCardTitle } = useMutation({
    mutationFn: (name:string) => changeCardName(memberData.memberId, data.id ,name),
  ...
  const { mutate: addCard } = useMutation({
    ...
    onSuccess: () => {
      setCard((prev: any[]) => prev.map(card =>
        card.id === data.id
          ? { 
              ...card, 
  ...
  return (
    <div className={searchWord && filteredItems.length === 0 ? cn("dashWrapNone") : cn("dashWrap")}>
    <div className={cn("title")}>
      ...
      <span>
        <Image src={moreBtn} alt="more" width={20} height={20} onClick={()=>addCard()}/>
        <div onClick={() => setDotDrop((prev) => !prev)} ref={ref}>
          <Image src={dot} alt="dot" width={20} height={20} />
          {dotDrop && (
            <DropDown
              item={[
                { color: "gray", text: "섹션 이름 바꾸기", click: () => setChangeName(false) },
