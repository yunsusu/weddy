...
import page3Img from "@/../public/images/testImg.jpg"
import page4Img from "@/../public/images/testImg.jpg"
...
export default function Home() {
  ...
  const handleGNBVisibility = (show: boolean) => {
    if (show) {
      document.documentElement.style.setProperty('--gnb-transform', 'translateY(0)');
      document.documentElement.style.setProperty('--gnb-opacity', '1');
    } else {
      document.documentElement.style.setProperty('--gnb-transform', 'translateY(-100%)');
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      console.log({
        ...
        nextColor: nextPageIndex < totalPages ? PAGE_COLORS[nextPageIndex] : null
      });
      ...
      if (nextPageIndex < totalPages) {
        ...
        if (scrollProgress >= 0.5) {
          ...
          console.log(`Transitioning to next color: ${nextColor}, progress: ${transitionProgress}`);
          ...
          console.log(`Staying at current color: ${PAGE_COLORS[currentPageIndex]}`);
    ...
    <div className={cn("pageContainer")} ref={containerRef}>
      ...
      <div className={cn("page")}>
        <div className={cn("secondPageContent")}>
          <div className={cn("secondPageSentence")}>
            ...
            <p>사용자 맞춤형 커스터마이징 기능을 통해 개인화된 체크리스트를 만들고, 필요한 정보를 한곳에 정리하여 효율적으로 관리할 수 있습니다.</p>
          </div>
      ...
      <div className={cn("page")}>
        ...
        <div className={cn("thirdPageSentence")}>
            ...
            <p>방대한 준비 항목을 체계적으로 정리하여 시작점부터 명확하게 제시하고, 결혼 준비의 복잡함을 덜어줍니다.</p>
          </div>
          <Image src={page2Img} alt="웨디 이미지" width={400} height={400} />
        </div>
      </div>
      ...
      <div className={cn("page")}>
        ...
        <div className={cn("foruthPageSentence")}>
            ...
            <p>필수 체크리스트를 통해 정보 검색 시간을 단축하고, 진행 상황을 직관적으로 파악하여 불필요한 시행착오 없이 체계적으로 결혼 준비를 할 수 있도록 돕습니다.</p>
          </div>
          <Image src={page2Img} alt="웨디 이미지" width={400} height={400} />
        </div>
      </div>
